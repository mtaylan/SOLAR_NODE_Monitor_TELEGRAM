...
##
#
# SOLAR NODE check & telegram alert script
# @mtaylan
##
## I moved all custom variables to a new file placed in .gitignore. Thank you very much to SOLAR Delegate crna_zmija who gave this advice.
#
. $HOME/SOLAR_NODE_Monitor_TELEGRAM/Solar-mon-telegram.conf
...
if [ "$1" == "test" ] ; then
        SEND_ALERT_FLAG_CPU=false
        MESSAGE="$(date) - [TEST] SOLAR node $HOSTNAME TEST message !!!.."
        echo " >>>> : $MESSAGE"
        telegram_send
        exit 0
fi
...
if [[ $ALIVE == "00" ]]
then
MESSAGE="$(date) - [SYSTEM] [OK] SOLAR  node  $HOSTNAME ALIVE !!!.."
        echo " >>>> : $MESSAGE"
        telegram_send
        exit 0
    fi
...
#Check SOLAR NODE  health status with API call
HTTP_CODE=$(curl -s -w '%{http_code}' --connect-timeout 5 --max-time 10 -o /dev/null http://dapi.solar.network/api/peers/$SOLAR_NODE_IP)
...
if [ "$CURL_STATUS" -eq 0 ]; then
        ...
        if [[ "$HTTP_CODE" -ne 200 ]] ; then
            check_alert_count
            MESSAGE="$(date) - [CRITICAL] [ALERT] SOLAR node is not running !!! #count:$COUNTER - returning http_code=$HTTP_CODE hostname=$HOSTNAME "
            echo " >>>> : $MESSAGE"
            telegram_send
        else
            echo " >>>> : SOLAR node $HOSTNAME is running!"

STATUS_HEALTHY=$(curl -sS http://dapi.solar.network/api/peers/$SOLAR_NODE_IP | jq -r '.data.latency')

        if [[ "$STATUS_HEALTHY" -lt $LATENCY_CRITICAL ]]; then
                MESSAGE="$(date) - [INFO] SOLAR node is healthy ! -  LATENCY(ms)=$STATUS_HEALTHY hostname=$HOSTNAME"
                echo " >>>> : $MESSAGE"
#               echo " >>>> : $STATUS_HEALTHY"
                if [ -f "$FILE" ]; then
                    echo "$FILE exists."
                    MESSAGE="$(date) - [INFO] [ALERT RESOLVED] SOLAR node is healthy again !!! -  LATENCY(ms)=$STATUS_HEALTHY hostname=$HOSTNAME"
                    rm $FILE
                ...
                check_alert_count
                MESSAGE="$(date) - [CRITICAL] [ALERT LATENCY] SOLAR node is not healthy !!! #count:$COUNTER -  LATENCY(ms)=$STATUS_HEALTHY hostname=$HOSTNAME"
                echo " >>>> : $MESSAGE"
                telegram_send
            fi
        fi
        ...
        check_alert_count
        MESSAGE="$(date) - [CRITICAL] [ALERT] SOLAR node is not running #count:$COUNTER - hostname=$(hostname)"
        echo " >>>> : $MESSAGE"
        telegram_send

fi

        BLOCK_STATUS_SOLAR=$(curl -sS http://dapi.solar.network/api/blockchain | jq .'[].block.height')
         BLOCK_STATUS_NODE=$(curl -sS http://dapi.solar.network/api/peers/$SOLAR_NODE_IP |  jq  '.data.height')
	 BLOCKS_BEHIND="$(($BLOCK_STATUS_SOLAR - $BLOCK_STATUS_NODE))"

          if [[ "$BLOCKS_BEHIND" -lt $MAX_BLOCKS_BEHIND ]]; then
                MESSAGE="$(date) - [INFO] $HOSTNAME Block Size within Acceptable Limit! - Block Height at $HOSTNAME only $BLOCKS_BEHIND Blocks behind SOLAR.network"
                echo " >>>> : Block Height at SOLAR.network=$BLOCK_STATUS_SOLAR"
...
if (( $(echo "$CPU_LOAD $CPU_LOAD_CRITICAL" | awk '{print ($1 > $2)}') )); then
        ...
        MESSAGE="$(date) - [CRITICAL] [ALERT FIRING] SOLAR node high CPU usage problem !!! #count:$COUNTER_CPU - Please check your processess $PROC - Linux SAR Total CPU Usage : $CPU_LOAD % - hostname=$HOSTNAME"
        ...
        if [ -f "$FILE_CPU" ]; then
                ...
                MESSAGE="$(date) - [INFO] [ALERT RESOLVED] SOLAR node normal CPU usage again !!! - Linux SAR Total CPU Usage : $CPU_LOAD % - hostname=$HOSTNAME"
                rm $FILE_CPU
